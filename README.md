<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Excalidraw" width="100%">
</p>

# Excalidraw

> Create hand-drawn Excalidraw JSON diagrams for architecture, flows, and sequences.
...
Creative requests often lose their intent between the initial brief, generation, export, and final quality check. **Excalidraw** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `excalidraw` names the capability directly—excalidraw—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Excalidraw**.
...
| What does it do? | Create hand-drawn Excalidraw JSON diagrams for architecture, flows, and sequences. |
...
![Excalidraw system map](assets/system-map.svg)

## Operation sequence

![Excalidraw actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Excalidraw](assets/example-output.svg)
...
> Using original or licensed sample material, create hand-drawn Excalidraw JSON diagrams for architecture, flows, and sequences. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
