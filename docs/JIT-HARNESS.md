...
This package expresses **Excalidraw** as a typed, task-adaptive agent harness. Its concrete capability remains: Create hand-drawn Excalidraw JSON diagrams for architecture, flows, and sequences. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Excalidraw |
| --- | --- | --- |
| Memory | `artifact_revision_history` | Retain architecture or flow brief plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `iterative_composition_plan` | Transform shape vocabulary into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through excalidraw scene graph while preserving stop conditions and user authority. |
